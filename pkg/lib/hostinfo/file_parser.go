...
func (fileInfoParser *fileInfoParser) Init() error {
	if _, err := os.Stat(msrFile); os.IsNotExist(err) {
		return errors.Wrapf(err, "Could not find hostname file '%s'", hostNameFile)
	}
...
func (fileInfoParser *fileInfoParser) parseHostName(hostInfo *model.HostInfo) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to open hostname file '%s'", hostNameFile)
	}

	defer func() {
		err = file.Close()
		if err != nil {
			log.Errorf("Failed close hostname file '%s': %s", hostNameFile, err.Error())
		}
	}()
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to read hostname file '%s'", hostNameFile)
	}
